...
// Leaderboard (public untuk user login)
Route::get('/leaderboard', function() {
    $leaderboard = \App\Models\Pretest::weeklyLeaderboard(50)->get();
    $prizes = \App\Models\Prize::forCurrentWeek()->orderBy('rank')->get();
    return view('leaderboard.full', compact('leaderboard', 'prizes'));
})->name('leaderboard')->middleware('auth');

// Admin Prize CRUD
Route::prefix('admin')->name('admin.')->group(function() {
    Route::resource('prizes', \App\Http\Controllers\Admin\PrizeController::class)
         ->middleware(['auth', 'admin']); // + tambahkan middleware admin jika ada
});